...
test('SK-feature-card Angular default — visual baseline', async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot('sk-feature-card-angular-default.png', { threshold: 0.02 });
});
...
test('SK-feature-card HTML default — visual baseline', async ({ page }) => {
  ...
  await page.waitForLoadState('domcontentloaded');
  await expect(page).toHaveScreenshot('sk-feature-card-html-default.png', { threshold: 0.02 });
});
