...
- 2026-05-03T16:57:31Z – opencode – shell_pid=1488840 – Review passed: workflow created with correct path filter (docs/architecture/assets/**), 5-min timeout, --check-only mode, SHA-pinned actions (check-action-pins.sh clean). @mermaid-js/mermaid-cli@11.14.0 pinned exactly. chromium-browser apt-install step correctly addresses puppeteer-config.json executablePath=/usr/bin/chromium. npm-audit-gate passes. Smoke-tests A/B/C verified locally: mmd drift → 1 file rejected; theme drift → 8 files rejected; clean state → OK 8/8. SVGs regenerated to match local Chromium baseline (idempotent check confirmed). Conventional-commit scopes used (ci, deps, fix, docs).
